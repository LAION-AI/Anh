...
def get_translation_set(text, threshold=0.75, langs = ["af", "am", "ar", "ast", "az", "ba", "be", "bg", "bn", "br", "bs", "ca", "ceb", \
                                         ...
                                         "wo", "xh", "yi", "yo", "zh", "zu"], return_original=False):
  ret = []
...
with open("/content/oa_v3_fixed_plus_safety.jsonl") as input:
  ...
  for l in input:
    ...
    if 'Python' in data['text']: continue # we need to make sure not to translate the functions and code themselves. only the comments
